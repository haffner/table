...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     1 sb_in(3040), t_in(3040,10,6)
      ...
      real dlam, weight
c
      common/array/wl(3040),fitflux(3040),cntrwl(22),output(22,5),
     1 nmb(22),stiwl(3040,5),dlam(41),weight(41,22)
c
c     -- Master table path
      data tdir/'/omps/dev/cseftor/'/
      data direc/
     1 'InstrumentTableGenerator/InstrumentTableGenerator/tbl/rad/'/
...
c     ------------------------
c
      open (10,file='lut_wav.dat',status='old')
      read (10,*) cntrwl
      close (10)
      do i = 1,22
...
c     -- read in solar flux data
      open (30,file='Flux_to_match_BDM_CRS_tc.dat',
     1 status='old')
...
c           -- open file corresponding to correct ozone profile
            if (ipres.ne.4)then
             open (33,file=tdir//direc//'/'//prf(iozprf)
     1        //'.'//atm(ipres)
     1        //'.rad.dat',status='old',form='unformatted')
            else 
             open (33,file=tdir//direc//'/'//prf(iozprf)
     1        //'.atm025.rad.dat',status='old',form='unformatted')
            endif
            print *,tdir//direc//prf(iozprf)//'.'//atm(ipres)//
     1       '.rad.dat'
c           -- read table values
